...
describe('Clothes DTO', () => {
  test('Should return bad request erro if name min size is less than 8', () => {
    const makeStub = () => {
      try {
        return new ClothesDTO(1, 'a', 1, 1, ['https://images.com']);
      } catch (err) {
        throw err;
      }
    };
  ...
  test('Should return bad request erro if id is less than 0', () => {
    const makeStub = () => {
      try {
        return new ClothesDTO(-1, 'aaaaaaaa', 1, 1, ['https://images.com']);
      } catch (err) {
        throw err;
      }
    };
  ...
  test('Should return bad request erro if images list is empty', () => {
    const makeStub = () => {
      try {
        return new ClothesDTO(1, 'aaaaaaaa', 1, -1, []);
      } catch (err) {
        throw err;
      }
    };
